...
class SchedulerGUI(tk.Tk):
    ...
    def setup_styles(self):
        self.dark_theme = {
            "bg": "#0A0A0A", "fg": "#EAEAEA", "header_bg": "#0F0F0F",
            "button_bg": "#1A1A1A", "button_fg": "#FFFFFF", "border": "#2A2A2A",
            "cell_bg": "#121212", "active_button": "#2A2A2A", 
            "primary_button": "#6B46C1", "primary_hover": "#553C9A",
            "secondary_button": "#4C1D95", "secondary_hover": "#5B21B6",
            "disabled_bg": "#1A1A1A", "disabled_fg": "#888888",
            "success_bg": "#10B981", "text_bg": "#000000",
            "canvas_bg": "#0A0A0A", "plot_bg": "#0F0F0F",
            "frame_bg": "#0A0A0A", "labelframe_bg": "#0F0F0F",
            "entry_bg": "#1A1A1A", "entry_fg": "#EAEAEA"
        }
        self.light_theme = {
            "bg": "#F0F0F0", "fg": "#000000", "header_bg": "#DCDCDC",
            "button_bg": "#E1E1E1", "button_fg": "#000000", "border": "#BDBDBD",
            "cell_bg": "#FFFFFF", "active_button": "#CFCFCF",
            "primary_button": "#3B82F6", "primary_hover": "#2563EB",
            "disabled_bg": "#CCCCCC", "disabled_fg": "#666666",
            "success_bg": "#10B981", "text_bg": "#FFFFFF",
            "canvas_bg": "#F5F5F5", "plot_bg": "#FFFFFF",
            "frame_bg": "#F0F0F0", "labelframe_bg": "#FFFFFF",
    ...
    def apply_theme(self):
        ...
        self.style.configure(".", background=theme["bg"], foreground=theme["fg"], 
                           bordercolor=theme["border"], fieldbackground=theme["entry_bg"])
        ...
        self.style.configure("TLabelframe.Label", background=theme["bg"], foreground=theme["fg"], font=("Segoe UI", 11, "bold"))
        self.style.configure("Header.TLabel", background=theme["header_bg"], foreground=theme["fg"], 
                           font=("Segoe UI", 12, "bold"), padding=(10, 5))
        self.style.configure("Time.TLabel", background=theme["header_bg"], foreground=theme["fg"], 
                           font=("Segoe UI", 11), padding=(10, 5))
        self.style.configure("Room.TLabel", background=theme["bg"], foreground=theme["fg"], 
                           font=("Segoe UI", 16, "bold"))
        self.style.configure("Cell.TLabel", background=theme["cell_bg"], foreground=theme["fg"], 
        ...
        self.style.configure("Primary.TButton", background=theme["primary_button"], foreground="#FFFFFF")
        self.style.map("Primary.TButton", background=[("active", theme["primary_hover"]),
                                                      ("disabled", theme["disabled_bg"])],
                     foreground=[("disabled", theme["disabled_fg"])])
        if "secondary_button" in theme:
            self.style.configure("Secondary.TButton", background=theme["secondary_button"], foreground="#FFFFFF")
            self.style.map("Secondary.TButton", background=[("active", theme["secondary_hover"])])
        ...
        self.style.configure("TCombobox", fieldbackground=theme["entry_bg"], foreground=theme["entry_fg"],
                           background=theme["button_bg"], bordercolor=theme["border"], 
                           arrowcolor=theme["fg"], selectbackground=theme["primary_button"])
        self.style.map("TCombobox", fieldbackground=[("readonly", theme["entry_bg"])],
                      foreground=[("readonly", theme["entry_fg"])])
        
        # Notebook styles
        self.style.configure("TNotebook", background=theme["bg"], bordercolor=theme["border"])
        self.style.configure("TNotebook.Tab", background=theme["button_bg"], foreground=theme["fg"],
                           padding=[10, 5])
        self.style.map("TNotebook.Tab", background=[("selected", theme["primary_button"])],
        ...
        if hasattr(self, 'status_text'):
            self.status_text.configure(bg=theme["text_bg"], fg=theme["fg"], insertbackground=theme["fg"])
        if hasattr(self, 'help_textbox'):
            self.help_textbox.configure(bg=theme["text_bg"], fg=theme["fg"], insertbackground=theme["fg"])
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(top_bar, text="🎓 AI-Hoshino Course Scheduler", 
                               font=("Segoe UI", 16, "bold"))
        title_label.pack(side="left", padx=10)
        
        theme_button = ttk.Button(top_bar, text="🌓 Toggle Theme", 
    ...
    def create_scheduler_page(self):
        ...
        load_button = ttk.Button(file_section, text="Load JSON File", 
                                command=self.load_file_for_scheduler, style="TButton")
        ...
        for text, value in algorithms:
            rb = ttk.Radiobutton(algo_section, text=text, variable=self.selected_algorithm, 
                                value=value, command=self.update_parameter_panel)
            rb.pack(anchor="w", pady=2)
        
        # Parameters Section
        self.param_section = ttk.LabelFrame(sidebar_frame, text="🔧 Parameters", padding="10")
        ...
        show_plots_cb = ttk.Checkbutton(options_section, text="Show Plots", 
        ...
        self.run_button = ttk.Button(button_frame, text="▶️ Run", 
                                     command=self.run_algorithm, style="Primary.TButton")
        self.run_button.pack(fill="x", ipady=10, pady=(0, 5))
        
        self.cancel_button = ttk.Button(button_frame, text="⏹️ Cancel", 
                                        command=self.cancel_algorithm, style="TButton",
                                        state=tk.DISABLED)
        ...
        # Status text area
        self.status_text = scrolledtext.ScrolledText(results_frame, height=10, 
                                                     font=("Consolas", 10), wrap=tk.WORD)
        self.status_text.pack(fill="both", expand=True)
        self.status_text.insert("1.0", "Load a file and select an algorithm to begin.\n")
        self.status_text.config(state=tk.DISABLED)
        
        # Plots Section (initially hidden)
        self.plots_frame = ttk.LabelFrame(content_frame, text="📊 Visualization", padding="10")
        ...
        plots_scrollbar = ttk.Scrollbar(self.plots_frame, orient="vertical", 
        ...
        self.plots_canvas.create_window((0, 0), window=self.plots_display_frame, anchor="nw")
        self.plots_display_frame.bind("<Configure>", 
    ...
    def create_schedule_viewer_page(self):
        # Info label at top
        info_label = ttk.Label(self.schedule_page, 
        ...
        schedule_scrollbar = ttk.Scrollbar(schedule_container, orient="vertical", 
        ...
        self.schedule_canvas.create_window((0, 0), window=self.schedule_display_frame, anchor="nw")
        self.schedule_display_frame.bind("<Configure>", self.on_schedule_frame_configure)
        
        # RIGHT: Plots display area with minimum width
        plots_container = ttk.LabelFrame(main_paned, text="📊 Visualization", padding="10", width=300)
        ...
        plots_v_scrollbar = ttk.Scrollbar(plots_scroll_frame, orient="vertical", 
                                         command=self.viewer_plots_canvas.yview)
        plots_v_scrollbar.grid(row=0, column=1, sticky="ns")
        
        # Horizontal scrollbar for plots
        plots_h_scrollbar = ttk.Scrollbar(plots_scroll_frame, orient="horizontal", 
                                         command=self.viewer_plots_canvas.xview)
        plots_h_scrollbar.grid(row=1, column=0, sticky="ew")
        
        # Configure canvas scrolling
        self.viewer_plots_canvas.configure(yscrollcommand=plots_v_scrollbar.set,
                                          xscrollcommand=plots_h_scrollbar.set)
        ...
        self.viewer_plots_canvas.create_window((0, 0), window=self.viewer_plots_display_frame, anchor="nw")
        self.viewer_plots_display_frame.bind("<Configure>", 
    ...
    def update_parameter_panel(self):
        ...
        if algo == 'hill_climbing':
            # Variant
            ttk.Label(self.param_frame, text="Variant:").grid(row=0, column=0, sticky="w", pady=2)
            variant_combo = ttk.Combobox(self.param_frame, textvariable=params['variant'], 
                                        state="readonly", width=18)
            variant_combo['values'] = ['stochastic', 'steepest_ascent', 'sideways_move', 'random_restart']
            ...
            ttk.Label(self.param_frame, text="Max Iterations:").grid(row=1, column=0, sticky="w", pady=2)
            ttk.Entry(self.param_frame, textvariable=params['max_iterations'], width=20).grid(row=1, column=1, pady=2)
            
            # Num Neighbors (for steepest_ascent and sideways_move)
            self.hc_param_widgets['num_neighbors_label'] = ttk.Label(self.param_frame, text="Num Neighbors:")
            self.hc_param_widgets['num_neighbors_label'].grid(row=2, column=0, sticky="w", pady=2)
            self.hc_param_widgets['num_neighbors_entry'] = ttk.Entry(self.param_frame, 
                                                                     textvariable=params['num_neighbors'], 
                                                                     width=20)
            self.hc_param_widgets['num_neighbors_entry'].grid(row=2, column=1, pady=2)
            
            # Max Sideways Moves (for sideways_move only)
            self.hc_param_widgets['max_sideways_label'] = ttk.Label(self.param_frame, text="Max Sideways:")
            self.hc_param_widgets['max_sideways_label'].grid(row=3, column=0, sticky="w", pady=2)
            self.hc_param_widgets['max_sideways_entry'] = ttk.Entry(self.param_frame, 
                                                                    textvariable=params['max_sideways_moves'], 
                                                                    width=20)
            self.hc_param_widgets['max_sideways_entry'].grid(row=3, column=1, pady=2)
            
            # Max Restarts (for random_restart only)
            self.hc_param_widgets['max_restarts_label'] = ttk.Label(self.param_frame, text="Max Restarts:")
            self.hc_param_widgets['max_restarts_label'].grid(row=4, column=0, sticky="w", pady=2)
            self.hc_param_widgets['max_restarts_entry'] = ttk.Entry(self.param_frame, 
                                                                    textvariable=params['max_restarts'], 
                                                                    width=20)
            ...
            # Initial Temperature
            ttk.Label(self.param_frame, text="Initial Temp:").grid(row=0, column=0, sticky="w", pady=2)
            ttk.Entry(self.param_frame, textvariable=params['initial_temp'], width=20).grid(row=0, column=1, pady=2)
            
            # Cooling Rate
            ttk.Label(self.param_frame, text="Cooling Rate:").grid(row=1, column=0, sticky="w", pady=2)
            ttk.Entry(self.param_frame, textvariable=params['cooling_rate'], width=20).grid(row=1, column=1, pady=2)
            
            # Max Iterations
            ttk.Label(self.param_frame, text="Max Iterations:").grid(row=2, column=0, sticky="w", pady=2)
            ttk.Entry(self.param_frame, textvariable=params['max_iterations'], width=20).grid(row=2, column=1, pady=2)
            
        elif algo == 'genetic':
            # Population Size
            ttk.Label(self.param_frame, text="Population Size:").grid(row=0, column=0, sticky="w", pady=2)
            ttk.Entry(self.param_frame, textvariable=params['population_size'], width=20).grid(row=0, column=1, pady=2)
            
            # Generations
            ttk.Label(self.param_frame, text="Generations:").grid(row=1, column=0, sticky="w", pady=2)
            ttk.Entry(self.param_frame, textvariable=params['generations'], width=20).grid(row=1, column=1, pady=2)
    ...
    def update_hc_param_states(self, *args):
        ...
        if self.hc_param_widgets['num_neighbors_entry']:
            self.hc_param_widgets['num_neighbors_entry'].config(state=state_num_neighbors)
            # Update label color to indicate disabled state
            fg_color = self.dark_theme['fg'] if enable_num_neighbors else self.dark_theme['disabled_fg']
            if self.dark_mode:
        ...
        if self.hc_param_widgets['max_sideways_entry']:
            ...
            fg_color = self.dark_theme['fg'] if enable_max_sideways else self.dark_theme['disabled_fg']
            if self.dark_mode:
        ...
        if self.hc_param_widgets['max_restarts_entry']:
            ...
            fg_color = self.dark_theme['fg'] if enable_max_restarts else self.dark_theme['disabled_fg']
            if self.dark_mode:
    ...
    def run_algorithm(self):
        ...
        if self.is_running:
            messagebox.showinfo("Running", "An algorithm is already running. Please wait.")
            return
    ...
    def _run_hill_climbing(self, objective_func, params):
        ...
        improvement_pct = (improvement / initial_penalty * 100) if initial_penalty > 0 else 0
        self.log_status(f"Improvement:      {improvement:.2f} ({improvement_pct:.1f}%)\n")
        ...
        if self.show_plots.get():
            try:
                fig = hc.plot_results(results, show=False)  # Don't show popup windows in GUI
    ...
    def _run_simulated_annealing(self, objective_func, params):
        ...
        if self.show_plots.get():
            try:
                figures = sa.plot_results(results, show=False)  # Don't show popup windows in GUI
                # Display each figure independently
                for i, fig in enumerate(figures):
                    title = f"SA Plot {i+1}: " + ("Objective Function" if i == 0 else "Acceptance Probability")
    ...
    def _run_genetic_algorithm(self, params):
        ...
        if self.show_plots.get() and fig is not None:
            try:
                self.after(0, lambda: self._display_plot(fig, "Genetic Algorithm Progress"))
            except Exception as e:
                self.log_status(f"\n⚠️ Could not generate plot: {e}\n")
    ...
    def _display_plot(self, fig, title):
        ...
        try:
            ...
            plot_frame = ttk.LabelFrame(self.viewer_plots_display_frame, text=title, padding="5")
    ...
    def display_all_schedules(self):
        ...
        if not self.state:
            no_schedule_label = ttk.Label(self.schedule_display_frame, 
        ...
        ttk.Label(summary_frame, text=f"📊 Final Penalty: {penalty:.2f}", 
                 font=("Segoe UI", 14, "bold")).pack(anchor="w")
        
        if self.algorithm_results:
            if 'iterations' in self.algorithm_results:
                ttk.Label(summary_frame, text=f"Iterations: {self.algorithm_results['iterations']}", 
    ...
    def create_schedule_grid(self, room):
        ...
        for hour in range(7, 18):
            row = hour - 6
            # Time label
            ttk.Label(grid_frame, text=f"{hour:02d}:00", style="Time.TLabel", 
                     anchor="center").grid(row=row, column=0, sticky="nsew", padx=1, pady=1)
            
            # Schedule cells for all days
            for day in range(1, 6):
                class_code = schedule.get((day, hour), "")
                ttk.Label(grid_frame, text=class_code, style="Cell.TLabel", 
                         anchor="center").grid(row=row, column=day, sticky="nsew", padx=1, pady=1)
